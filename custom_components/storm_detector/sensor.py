"""Sensor entities for Storm Detector."""
...
async def async_setup_entry(hass: Any, config_entry: Any, async_add_entities: Any) -> None:
    ...
    async_add_entities(
        [
            StormDetectorLevelSensor(coordinator, config_entry),
            StormDetectorSummarySensor(coordinator, config_entry),
            StormDetectorMaxDbzSensor(coordinator, config_entry),
            StormDetectorCoreDistanceSensor(coordinator, config_entry),
            StormDetectorLightningDistanceSensor(coordinator, config_entry),
            StormDetectorFrameAgeSensor(coordinator, config_entry),
            StormDetectorLastErrorSensor(coordinator, config_entry),
        ]
    )


class StormDetectorSensorBase(CoordinatorEntity[Any], SensorEntity):
    """Common helpers shared by all Storm Detector sensors."""
    ...
    def __init__(self, coordinator: Any, config_entry: Any, *, key: str, title: str, unit: str | None = None) -> None:
        ...
        self._attr_translation_key = {
            ATTR_CORE_DISTANCE_KM: "core_distance",
            ATTR_FRAME_AGE_SECONDS: "frame_age",
            ATTR_LIGHTNING_DISTANCE_KM: "lightning_distance",
        }.get(key, key)
        self._unit = unit
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        return {
            ...
            ATTR_EVIDENCE_KIND: data.get(ATTR_EVIDENCE_KIND),
            ...
            ATTR_CORE_WATCH_DISTANCE_KM: data.get(ATTR_CORE_WATCH_DISTANCE_KM),
            ATTR_CORE_WARNING_DISTANCE_KM: data.get(ATTR_CORE_WARNING_DISTANCE_KM),
            ATTR_CORE_URGENT_DISTANCE_KM: data.get(ATTR_CORE_URGENT_DISTANCE_KM),
            ...
            ATTR_STORM_CORES: data.get(ATTR_STORM_CORES) or [],
...
class StormDetectorLevelSensor(StormDetectorSensorBase):
    """Overall storm attention level."""
...
class StormDetectorSummarySensor(StormDetectorSensorBase):
...
class StormDetectorMaxDbzSensor(StormDetectorSensorBase):
...
class StormDetectorCoreDistanceSensor(StormDetectorSensorBase):
...
class StormDetectorLightningDistanceSensor(StormDetectorSensorBase):
...
class StormDetectorFrameAgeSensor(StormDetectorSensorBase):
...
class StormDetectorLastErrorSensor(StormDetectorSensorBase):
